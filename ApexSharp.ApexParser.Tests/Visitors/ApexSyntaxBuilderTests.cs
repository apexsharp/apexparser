...
namespace ApexSharp.ApexParser.Tests.Visitors
{
    ...
    public class ApexSyntaxBuilderTests : TestFixtureBase
    {
        ...
        [Test]
        public void ApexExceptionsWithAutoGeneratedConstructorsGetCleanedUp()
        {
            // empty C# exception class translates to empty Apex class
            Check("class MyX : Exception { }", "class MyX extends Exception { }");

            // empty constructor auto-generated by ApexParser gets removed
            Check(@"class MyX : Exception {
                    [ApexSharpGenerated] public MyX(string message) : base(message) { }
                }",
                "class MyX extends Exception { }");

            // non-exception classes are left intact
            Check(@"class MyX {
                    public MyX(string message) { }
                }",
                @"class MyX {
                    public MyX(String message) { }
                }");

            // non-empty constructors are also left intact
            Check(@"class MyX : Exception {
                    public MyX(string message) { int a = 10; }
                }",
                @"class MyX extends Exception {
                    public MyX(String message) { Integer a = 10; }
                }");
        }
    }
}
