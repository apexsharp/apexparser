...
namespace ApexParser.Visitors
{
    public class CSharpCodeGenerator : ApexCodeGeneratorBase
    {
        ...
        protected override void AppendClassMembers(ClassDeclarationSyntax node, IEnumerable<MemberDeclarationSyntax> classMembers = null)
        {
            classMembers = classMembers ?? node.Members;

            // special case: add missing constructors for the classes derived from Exception
            if (node.BaseType != null &&
                node.BaseType.Identifier == ApexKeywords.Exception)
            {
                var existingConstructors = classMembers.OfType<ConstructorDeclarationSyntax>();
                var autoGeneratedConstructors = GetExceptionConstructors(node);
                var missingConstructors = GetMissingConstructors(existingConstructors, autoGeneratedConstructors);
                classMembers = missingConstructors.Concat(classMembers);
            }

            base.AppendClassMembers(node, classMembers);
        }

        internal IEnumerable<ConstructorDeclarationSyntax> GetExceptionConstructors(ClassDeclarationSyntax node)
        {
            var modifiers = new[] { "public" }.ToList();
            var returnType = new TypeSyntax(node.Identifier);
            var emptyBody = new BlockSyntax();
            var autoGenerated = new List<AnnotationSyntax>
            {
                new AnnotationSyntax("ApexSharpGenerated"),
            };

            // default constructor — no parameters
            yield return new ConstructorDeclarationSyntax
            {
                Annotations = autoGenerated,
        ...
        internal IEnumerable<ConstructorDeclarationSyntax> GetMissingConstructors(
            IEnumerable<ConstructorDeclarationSyntax> existingConstructors,
            IEnumerable<ConstructorDeclarationSyntax> allConstructors)
        {
            existingConstructors = existingConstructors.EmptyIfNull().ToArray();
            allConstructors = allConstructors.EmptyIfNull().ToArray();

            bool IsSame(ConstructorDeclarationSyntax leftC, ConstructorDeclarationSyntax rightC)
            {
                ...
                if (leftC.Parameters.Count != rightC.Parameters.Count)
                {
                    return false;
                }

                foreach (var param in leftC.Parameters.Zip(rightC.Parameters, (left, right) => new { left, right }))
                {
                    if (param?.left?.Type?.Identifier?.ToLower() != param?.right?.Type?.Identifier?.ToLower())
                    {
                        return false;
                    }
                }
            ...
            foreach (var cd in allConstructors)
            {
                if (existingConstructors.All(cc => !IsSame(cc, cd)))
                {
                    yield return cd;
                }
            }
